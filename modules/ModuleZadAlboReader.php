...
/**
 * Class ModuleZadAlboReader
 *
 * Front end module "Albo Reader".
 *
 * @copyright Antonello Dessì 2015
 * @author    Antonello Dessì
 * @package   zad_albo
 */
class ModuleZadAlboReader extends \Module {
	 ...
	 * @var string
	 */
	protected $strTemplate = 'zadar_message';
	...
	public function generate() {
      ...
      $template->wildcard = '### ' . utf8_strtoupper($GLOBALS['TL_LANG']['FMD']['zad_albo_reader'][0]) . ' ###';
    ...
    // set template
    $this->Template = new \FrontendTemplate('zadar_show');
    // get document
    $doc = \ZadAlboDocumentModel::getPublishedDocument($category, $id);
    if ($doc === null) {
  ...
  /**
    ...
    $doc = \ZadAlboDocumentModel::getPublishedDocumentByFile($category, $uuid);
    if ($doc === null) {
      // error no file
      $this->errorMessage($GLOBALS['TL_LANG']['tl_zad_albo']['err_id']);
      return;
    ...
    // set template
    $this->Template = new \FrontendTemplate('zadar_list');
    // get search subject
    ...
    $limit = null;
    $offset = null;
    $total = \ZadAlboDocumentModel::countPublishedDocuments(($category == 0) ? $category_ids : $category, $subject_terms);
    ...
    // get docs
    $data = array();
    $docs = \ZadAlboDocumentModel::findPublishedDocuments(($category == 0) ? $category_ids : $category, $subject_terms, $offset, $limit);
    $index = 0;
    while ($docs->next()) {
      // number
    ...
    $this->Template->lbl_publishedlist = $GLOBALS['TL_LANG']['tl_zad_albo']['lbl_publishedlist'];
    $this->Template->lbl_number = $GLOBALS['TL_LANG']['tl_zad_albo']['lbl_number'];
    ...
    $this->Template = new \FrontendTemplate('zadar_message');
    if ($message == null) {
